...
You are not trying to fully understand it here — you are trying to carve the space into distinct sub-questions so your subagents don't all grep the same file. Sketch a quick coverage map: ~n meaningfully different angles on this area (plus 1–2 variance seats), and what's explicitly out of scope. That map is what makes the parallelism pay off instead of producing ten overlapping reports.

If the area turns out small enough that one or two reads answer it, just answer it directly — don't run /exec for a one-liner.

## 2. Dispatch agents

Spawn subagents in parallel with the Task tool — **one per angle** — scaling the count to the breadth you found in step 1 (default ~5; more only if the area is genuinely sprawling). Flooding a small area with agents mostly buys redundant work and burns tokens for no extra signal.
...
- **Variance seats** — reserve 1–2 agents for angles a straight investigation skips: "what's fragile or could go wrong here?", "is there dead, duplicated, or orphaned code around X?", "what's surprising or inconsistent?". This is where the orchestration beats a checklist.
...
- **Open questions**: what the orchestration couldn't determine, so it doesn't masquerade as settled.
...
## Param reference
- `n=<number>` — override the subagent count (default ~5).
- `plan` / `plan mode` — synthesize into a plan instead of implementing.
- Topic is inferred from the rest of the prompt.

## Example invocations
- `/exec n=10 how does authentication work?`
- `/exec find all places we use InstancedGeometry n=5`
- `/exec getting this error, investigate: <paste error>`
- `/exec plan — how should we refactor the payment module?`
